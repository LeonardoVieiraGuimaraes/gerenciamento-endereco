<#--
  Cópia do index.ftl do tema keycloak.v2 (Admin Console) apenas para traduzir os
  textos que ficam FIXOS no HTML e por isso não saem no bundle de mensagens:
  a frase de carregamento, o aviso de JavaScript e o rótulo do spinner.

  Toda a lógica FreeMarker (importmap, dark mode, scripts, bloco "environment")
  foi mantida igual à original — se um dia o Keycloak for atualizado, vale
  comparar este arquivo com o original da nova versão antes de subir.
-->
<!doctype html>
<html lang="pt-BR">
  <head>
    ...
    <meta name="description" content="${properties.description!'Console de administração do Keycloak.'}">
    <title>${properties.title!'Console de Administração'}</title>
    <style>
      body {
        margin: 0;
      }
    ...
    <div id="app">
      <main class="container">
        <div class="keycloak__loading-container">
          <svg class="pf-v5-c-spinner pf-m-xl" role="progressbar" aria-valuetext="Carregando..." viewBox="0 0 100 100" aria-label="Conteúdo">
            <circle class="pf-v5-c-spinner__path" cx="50" cy="50" r="45" fill="none"></circle>
          </svg>
          <div>
            <p id="loading-text">Carregando o Console de Administração</p>
          </div>
        </div>
      </main>
    </div>
    <noscript>É necessário JavaScript para usar o Console de Administração.</noscript>
